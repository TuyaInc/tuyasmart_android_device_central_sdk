...
| mSerialPort  | 串口终端（无zigbee模块则无需配置这项）                                                     |
| mTempDir     | 临时文件目录，该目录要在app中创建  （无zigbee模块则无需配置这项）                           |
| mBinDir      | bin文件目录，该文件夹下不要存放其他文件，该目录要在app中创建（无zigbee模块则无需配置这项）  |
| mIsCTS       | 是否带流控（无zigbee模块则无需配置这项）                                                    |
